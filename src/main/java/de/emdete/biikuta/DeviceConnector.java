...
public class DeviceConnector implements Constants {
		...
		try {
			...
			if (phoneUuids != null) {
				for (ParcelUuid uuid : phoneUuids) {
					U.info(device.getName() + ": " + uuid.toString());
					result.add(uuid);
				}
			}
		}
		catch (NoSuchMethodException e) {
			U.info("getDeviceUuids() failed", e);
		}
	...
	public static BluetoothSocket createRfcommSocket(BluetoothDevice device) {
		...
		try {
			...
			U.info("createRfcommSocket() failed", e);
		}
		catch (InvocationTargetException e) {
			U.info("createRfcommSocket() failed", e);
		}
	...
	public synchronized void connect() {
		U.info("connect to: " + connectedDevice);
		if (mState == R.id.STATE_CONNECTING) {
			if (mConnectThread != null) {
				U.info("cancel mConnectThread");
				mConnectThread.cancel();
				mConnectThread = null;
			}
		}
		if (mConnectedThread != null) {
			U.info("cancel mConnectedThread");
			mConnectedThread.cancel();
			mConnectedThread = null;
		}
		mConnectThread = new ConnectThread(connectedDevice);
		mConnectThread.start();
		setState(R.id.STATE_CONNECTING);
	}
	...
	private synchronized void setState(int state) {
		U.info("setState() " + mState + " -> " + state);
		mState = state;
		mHandler.obtainMessage(R.id.MESSAGE_STATE_CHANGE, state, -1).sendToTarget();
	}
	...
	public synchronized void connected(BluetoothSocket socket) {
		...
		setState(R.id.STATE_CONNECTED);
		Message msg = mHandler.obtainMessage(R.id.MESSAGE_DEVICE_NAME, deviceName);
	...
	public void write(byte[] data) {
		ConnectedThread r;
		synchronized (this) {
			if (mState != R.id.STATE_CONNECTED) return;
			r = mConnectedThread;
		}
	...
	private class ConnectThread extends Thread {
		...
		public ConnectThread(BluetoothDevice device) {
			U.info("create ConnectThread");
			mmDevice = device;
			mmSocket = createRfcommSocket(mmDevice);
		}

		public void run() {
			U.info("ConnectThread run");
			btAdapter.cancelDiscovery();
			if (mmSocket == null) {
				U.info("unable to connect to device, socket isn't created");
				connectionFailed();
				return;
			}
			try {
				...
				try {
					...
					U.info("unable to close() socket during connection failure", e2);
				}
				connectionFailed();
				return;
			}
		...
		public void cancel() {
			U.info("ConnectThread cancel");
			if (mmSocket == null) {
				U.info("unable to close null socket");
				return;
			}
			try {
				mmSocket.close();
				...
				U.info("close() of connect socket failed", e);
			}
		}
	}
	...
	private class ConnectedThread extends Thread {
		...
		public ConnectedThread(BluetoothSocket socket) {
			...
			try {
				...
				U.info("temp sockets not created", e);
			}
			mmInStream = tmpIn;
			mmOutStream = tmpOut;
		}
		...
		public void run() {
			...
			while (true) {
				try {
					...
					if (readed.contains("\n")) {
						mHandler.obtainMessage(R.id.MESSAGE_READ, bytes, -1, readMessage.toString()).sendToTarget();
						readMessage.setLength(0);
					}
				}
				catch (IOException e) {
					U.info("disconnected", e);
					connectionLost();
					break;
				}
			}
		...
		public void writeData(byte[] chunk) {
			try {
				...
				mmOutStream.flush();
				mHandler.obtainMessage(R.id.MESSAGE_WRITE, -1, -1, chunk).sendToTarget();
			}
			catch (IOException e) {
				U.info("Exception during write", e);
			}
		}
		...
		public void write(byte command) {
			...
			try {
				...
				mHandler.obtainMessage(R.id.MESSAGE_WRITE, -1, -1, buffer).sendToTarget();
			}
			catch (IOException e) {
				U.info("Exception during write", e);
			}
